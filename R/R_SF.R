...
#'
RSF_convert <- function(rsf_file1, output_name = rsf_file1,crop = FALSE,layout_csv = NULL, crop_buffer = 700, twelve_S = FALSE, rsf_file2 = NULL, shear_out = FALSE, three_D = TRUE, layer_H = NULL){
...
# read and parse data -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
  ...
  if(!is.null(layout_csv)){
    ...
    layout <- layout[complete.cases(layout),]
      ...
      if(length(layout) != 2){
        stop("Check that the layout file is a *.csv file with two columns using "," as delimiter")
      }else if(min(layout[,1]) < rsf_H1[,min(X)] | max(layout[,1]) > rsf_H1[,max(X)] | min(layout[,2]) < rsf_H1[,min(Y)] | max(layout[,2]) > rsf_H1[,max(Y)]){
...
# Crop files -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
  ...
  if(twelve_S == TRUE){
    ...
    rsf_H1_12 <- S36_to_S12_RSF(rsf_H1)
      ...
      write.csv(bench_table, paste("benchmark_table_", output_name,".csv", sep = ""))

      pdf(file = paste("benchmark_graph_", output_name,".pdf", sep = ""))
      hist(bench_table$ratio12_36, 10, col = "green", main = "AEP ratio: 12 sectors versus 36 sectors", xlab = "Ratio", breaks = seq(0.9,1.1,0.005))
    ...
    output_name <- paste("[12S_",round(Ratio*100-100,1),"]",output_name, sep = "")
    ...
    #Write_RSF(shear_table,shear_name)
    #Shear2pngV2(shear_table)
    }
...
RSF_3D <- function(rsf_file1, rsf_file2,layout_csv, output_name = rsf_file1, twelve_S = TRUE){
    # Objectif: based on 3 inputs files (36 Sect H1 rsf, 36 Sect H2 rsf, and layout (*.csv)), this "Master"
    # function generate a shear map and 3Dimensional rsf files).
    # this "Master" funtion taking into account the input files as direct input (and then call the other sub functions.)
    # Desciption of the inputs:
    #        - rsf_file1  : 36 sectors rsf file at the unique heigh of H1
    #        - rsf_file2  : 36 sectors rsf file at the unique heigh of H2
    #        - layout_csv : csv file containing the X and Y position in the two first columns
    #        - output_name : optional argument. It will be used as root for naming the out files (can be project name for example)
    #        - twelve_S : optional argument. Boolean that define if the coverstion from 36 to 12 need to be done (default = TRUE)
...
# read and parse data -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
    ...
    if(abs(H1-H2) < 10){
        stop(paste("The height difference from the two rsf file are too low (<10m)",H1,H2))
    }else if(abs(H1-H2) < 20){
        warning(paste("the heigth difference is below 20m",H1,H2))
    }

# Crop files -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
    rsf_H1 <- Crop_RSF(rsf_H1, layout, 500)
    rsf_H2 <- Crop_RSF(rsf_H2, layout, 500)
...
# Shear map -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
    #shear_table <- Shear_RSF(rsf_H2,rsf_H1)
    #shear_name <- paste("[shear_map]",output_name,".rsf", sep = "")
    #Write_RSF(shear_table,shear_name)
...
# 3D rsf -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
    ...
    # creation of the vector containing the different calculation heights
        ...
        layer_H <- seq(0,200,10)
        layer_H <-layer_H[layer_H > min(H1,H2) & layer_H < max(H1,H2)]
        layer_H <- c(100,117,125,137,147,155) #+/- Standard Hub Height
        #layer_H <- c(54,117,180)

    # creation of the 3 dimensionals rsf
        threeD_table <- Interpol_RSF(rsf_H1, rsf_H2, layer_H)
        threeD_name <- paste("[3D]",output_name,".rsf", sep = "")
        Write_RSF(threeD_table ,threeD_name)
...
#' RSF36_2_12 Function
#'
#' Based on 2 inputs files (36 Sect rsf, and layout (*.csv)), this "Master" function generate a "Cropped" rsf (700m around the turbine positions)
#' and also a "Cropped + 12 sectors"" rsf. this "Master" function taking into account the input files as direct input (and then call the other sub functions)
...
RSF36_2_12 <- function(rsf_file,layout_csv,output_name = rsf_file1){
    # Objectif: based on 2 inputs files (36 Sect rsf, and layout (*.csv)), this "Master"
    # function generate a Crop rsf (700m around the turbine positions) map and a Crop+12 sectors rsf.
    # this "Master" funtion taking into account the input files as direct input (and then call the other sub functions.)
    # Desciption of the inputs:
    #        - rsf_file1  : 36 sectors rsf file at the unique heigh of H1
    #        - layout_csv : csv file containing the X and Y position in the two first columns
    #        - output_name : optional argument. It will be used as root for naming the output files (can be project name for example)
    #        - twelve_S : Boolean that define if the coverstion from 36 to 12 need to be done (TRUE as default)
...
library("data.table")
  ...
  RSF36 <- Read_RSF(rsf_file)
  layout <- read.csv(layout_csv)
  H <- unique(RSF36[,Height])

  RSF36 <- Crop_RSF(RSF36,layout,700)
  Write_RSF(RSF36,paste("[Crop][",H,"m]",output_name,".rsf",sep = ""))



  RSF12 <- S36_to_S12_RSF(RSF36)
  bench_table <- Bensh_RSF(RSF36,RSF12, layout, Gen_power_curve())
  Ratio <- bench_table[, mean(ratio12_36)]

  write.csv(bench_table, paste("Bench_table_", output_name,".csv", sep = ""))
  Write_RSF(RSF12,paste("[12S_",round(Ratio*100-100,1),"]","[Crop][",H,"m]",output_name,".rsf", sep = ""))
...
Read_RSF <- function(Input_file){
  ...
  library("data.table")         ## use forthe output format
      ...
      if(nchar(line)>400){          ## it means it should be a 36 sectors (actually 36S should be exactly 540)
            ...
            RSF_Col_Name <- c("Label","X","Y","Z","Height","Aave","k","Blank","Sector")
            RSF_Col_Name <- c(RSF_Col_Name,paste(c("F36_","A36_","k36_"),rep(seq(0,350,10),each = 3),sep = ""))



      }else{                        ##if less than 400, then it means it should be a 12 sectors (actually 12S should be exactly 228)
            RSF_Col_Format <- c(10, 10, 10, 8, 5, 6, 5, 15, 3, 4, 4, 5, 4, 4, 5, 4, 4, 5, 4,
                                  4, 5, 4, 4, 5, 4, 4, 5, 4, 4, 5, 4, 4, 5, 4, 4, 5, 4, 4, 5, 4,
                                  4, 5, 4, 4, 5)

            RSF_Col_Name <- c("Label","X","Y","Z","Height","Aave","k","Blank","Sector")
            RSF_Col_Name <- c(RSF_Col_Name,paste(c("F12_","A12_","k12_"),rep(seq(0,330,30),each = 3),sep = ""))
      }

      RSF_table <- data.table(read_fwf(Input_file, fwf_widths(RSF_Col_Format, col_names = RSF_Col_Name)))
...
#' load into memory an rsf file into the object rsf_166:
#' rsf_166 <- Read_RSF("RSF-windresource-CFD_Aldermyrberget 166m.rsf")
Crop_RSF <- function(rsf,Xmax, Xmin, Ymax, Ymin){  # (A,B,C,D, could be read as "Xmax, Xmin, Ymax, Ymin", or just "Layout_Table, Buffer in [m]")
...
S36_to_S12_RSF <- function(RSF36){
  ...
  RSF12[,A12_0 := round((A36_350*F36_350 + A36_0*F36_0 + A36_10*F36_10)/F12_0,0)]
  RSF12[,k12_0 := round((k36_350*F36_350 + k36_0*F36_0 + k36_10*F36_10)/F12_0,0)]

  RSF12[,F12_30 := F36_20 + F36_30 + F36_40]
  RSF12[,A12_30 := round((A36_20*F36_20 + A36_30*F36_30 + A36_40*F36_40)/F12_30,0)]
  RSF12[,k12_30 := round((k36_20*F36_20 + k36_30*F36_30 + k36_40*F36_40)/F12_30,0)]

  RSF12[,F12_60 := F36_50 + F36_60 + F36_70]
  RSF12[,A12_60 := round((A36_50*F36_50 + A36_60*F36_60 + A36_70*F36_70)/F12_60,0)]
  RSF12[,k12_60 := round((k36_50*F36_50 + k36_60*F36_60 + k36_70*F36_70)/F12_60,0)]

  RSF12[,F12_90 := F36_80 + F36_90 + F36_100]
  RSF12[,A12_90 := round((A36_80*F36_80 + A36_90*F36_90 + A36_100*F36_100)/F12_90,0)]
  RSF12[,k12_90 := round((k36_80*F36_80 + k36_90*F36_90 + k36_100*F36_100)/F12_90,0)]

  RSF12[,F12_120 := F36_110 + F36_120 + F36_130]
  RSF12[,A12_120 := round((A36_110*F36_110 + A36_120*F36_120 + A36_130*F36_130)/F12_120,0)]
  RSF12[,k12_120 := round((k36_110*F36_110 + k36_120*F36_120 + k36_130*F36_130)/F12_120,0)]

  RSF12[,F12_150 := F36_140 + F36_150 + F36_160]
  RSF12[,A12_150 := round((A36_140*F36_140 + A36_150*F36_150 + A36_160*F36_160)/F12_150,0)]
  RSF12[,k12_150 := round((k36_140*F36_140 + k36_150*F36_150 + k36_160*F36_160)/F12_150,0)]

  RSF12[,F12_180 := F36_170 + F36_180 + F36_190]
  RSF12[,A12_180 := round((A36_170*F36_170 + A36_180*F36_180 + A36_190*F36_190)/F12_180,0)]
  RSF12[,k12_180 := round((k36_170*F36_170 + k36_180*F36_180 + k36_190*F36_190)/F12_180,0)]

  RSF12[,F12_210 := F36_200 + F36_210 + F36_220]
  RSF12[,A12_210 := round((A36_200*F36_200 + A36_210*F36_210 + A36_220*F36_220)/F12_210,0)]
  RSF12[,k12_210 := round((k36_200*F36_200 + k36_210*F36_210 + k36_220*F36_220)/F12_210,0)]

  RSF12[,F12_240 := F36_230 + F36_240 + F36_250]
  RSF12[,A12_240 := round((A36_230*F36_230 + A36_240*F36_240 + A36_250*F36_250)/F12_240,0)]
  RSF12[,k12_240 := round((k36_230*F36_230 + k36_240*F36_240 + k36_250*F36_250)/F12_240,0)]

  RSF12[,F12_270 := F36_260 + F36_270 + F36_280]
  RSF12[,A12_270 := round((A36_260*F36_260 + A36_270*F36_270 + A36_280*F36_280)/F12_270,0)]
  RSF12[,k12_270 := round((k36_260*F36_260 + k36_270*F36_270 + k36_280*F36_280)/F12_270,0)]

  RSF12[,F12_300 := F36_290 + F36_300 + F36_310]
  RSF12[,A12_300 := round((A36_290*F36_290 + A36_300*F36_300 + A36_310*F36_310)/F12_300,0)]
  RSF12[,k12_300 := round((k36_290*F36_290 + k36_300*F36_300 + k36_310*F36_310)/F12_300,0)]

  RSF12[,F12_330 := F36_320 + F36_330 + F36_340]
  RSF12[,A12_330 := round((A36_320*F36_320 + A36_330*F36_330 + A36_340*F36_340)/F12_330,0)]
  RSF12[,k12_330 := round((k36_320*F36_320 + k36_330*F36_330 + k36_340*F36_340)/F12_330,0)]
...
# write an rsf table ("RSF") in memory to a *.rsf file ("output_file_name")
  ...
  write.fwf(RSF,output_file_name,colnames = FALSE, width = RSF_Col_Format, sep = "", na = "40")    #na need a value (other wise windpro will trow an error)
...
Shear_RSF <- function(rsf_H1,rsf_H2){
    ...
    Shear <- rsf_H1[,.(Label,X,Y,Z,Height,Aave,k,Blank,Sector)]  #  initialization shear <= first columns of the rsf

    Shear[,':=' (Height = 999, Aave = log(rsf_H1[,Aave]/rsf_H2[,Aave])/log(H1/H2), Sector = 0)] #change of the All sector (999) part
    Buffer <- Shear[,.(Label,X,Y,Z,Height,Aave,k,Blank,Sector)]

    for(i in 1:N_Sector){    # fil sectorwise wind shear
...
Prod_RSF <- function(rsf, Xp, Yp,power_curve){
    ...
    }else if(Xp < rsf[,min(X)] | Xp > rsf[,max(X)] | Yp < rsf[,min(Y)] | Yp > rsf[,max(Y)] ){
...
Bensh_RSF <- function(rsf36, rsf12, layout, power_curve){
  ...
  }else if(min(layout[,1]) < rsf36[,min(X)] | max(layout[,1]) > rsf36[,max(X)] | min(layout[,2]) < rsf36[,min(Y)] | max(layout[,2]) > rsf36[,max(Y)]){
  ...
  for (i in 1:nrow(layout)) {
    results[i,':=' (AEP_36 = Prod_RSF(rsf36,X,Y,power_curve),AEP_12 = Prod_RSF(rsf12,X,Y,power_curve)) ]
  }
...
Gen_power_curve <- function(turbine = "V117-3.45"){
  ...
  PC <- data.frame("wind_speed[m/s]" = seq(0, 40, 0.5),
      "Power[kW]" = c(
        0,
        0,
        0,
...
Interpol_RSF <- function(rsf_H1,rsf_H2,layer_H){
  # create a rsf file at the heigth(s) provided in the vector "layer_H", based on the interpolation of rsf at H1 and H2. it assumes a shear profile of the wind speed.
   ...
   # check the coherence of the grids (X,Y) of the two RSF. In order to work with column operations, grids should be exactly similar.
    if(min(rsf_H1[,X] == rsf_H2[,X])==0 | min(rsf_H1[,Y] == rsf_H2[,Y])==0 ){
  ...
  interpol_table[,':=' (Aave = log(rsf_H1[,Aave]/rsf_H2[,Aave])/log(H1/H2), k = (rsf_H1[,k]-rsf_H2[,k])/(H1-H2))]   #the overall gradient columns (not sectorwise)
  ...
  setnames(interpol_table,"k","Slope_k")
    ...
    for(H3 in layer_H){  # to merge with the code above where layer_H = one height only)

      rsf_H3 <- data.table(rsf_H1)
      rsf_H3[,':='(Aave = round(rsf_H1[,Aave]*(H3/H1)^interpol_table[,Shear],2),
...
Shear2png <- function(RSF_shear){
    ...
    for(s in N_Sector){
        ...
        pwg_name <- paste("shear_",s,".pgw",sep = "")          #same name than the PNG (but with pwg extension)
        cat(pxl_size,"\n0\n0\n-",pxl_size,"\n",first_pxl_x,"\n",first_pxl_y, file = pwg_name,sep = "") #content in pwg is always the same whatever the sector
        ...
        plot <- ggplot(shear,aes(X,Y,z=ShearAve))+
            ...
            labs(x = NULL, y = NULL) +
            theme_nothing() +
            stat_contour(aes(colour=..level..), breaks=c(0,0.6),show.legend = NA)+ guides(colour=FALSE)+
            annotation_raster(mypng, ymin = first_pxl_y- pxl_size*99 ,    # insert legend
                              ymax= first_pxl_y,
                              xmin = first_pxl_x,
        ...
        dev.copy(png, file = png_name, width=img_size_x, height = img_size_y )
        dev.off()
    }
}
...
Shear2pngV2 <- function(RSF_shear){
  ...
  for(s in N_Sector){
    ...
    plot <- ggplot(shear,aes(X,Y,z=ShearAve))+
      ...
      theme_nothing() +
      stat_contour(aes(colour=..level..), breaks=c(0,0.6),show.legend = NA)+ guides(colour=FALSE)+
      annotation_raster(mypng, ymin = first_pxl_y- pxl_size*99 ,    # insert legend (existing picture load in memory)
                        ymax= first_pxl_y,
                        xmin = first_pxl_x,
                        xmax = first_pxl_x+pxl_size*46)
...
ShearTable <- function(RSF_shear, output_name = "sheartable.csv"){
        ...
        mat <- spread(data = RSF_shear,key = Direction,value = ShearAve)
        mat <- subset(mat, select = -c(Label,Z,k,Blank,Sector))            #remove unnecessary columns
        mat <- round(x = mat, digits = 2)
        write.csv2(x = mat,file = output_name,  row.names = FALSE)
        mat
...
ShearExtract <- function(shear_csv,layout_csv, output_name = "shear_VSC_input.shr"){
      # Extract shear at turbine locations, and creat a *.shr file for VSC (use as CFD input)
        ...
        for (i in 1:nrow(layout)) {
                Xp <- layout[i,1]
                Yp <- layout[i,2]
                buffer <- shear[which.min(abs(X - Xp) + abs(Y - Yp))]
...
Vizu_RSF <- function(rsf){
  ...
  manipulate(p+geom_raster(aes(fill= subset(rsf, select = variable)[[1]]))+  #[[1]] --> needed because of input format condition of geom_raster
